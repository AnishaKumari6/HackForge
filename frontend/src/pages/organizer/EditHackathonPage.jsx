...
const EditHackathonPage = () => {
  ...
  useEffect(() => {
    hackathonService.getMyHackathons({ limit: 100 }).then(({ hackathons }) => {
      const h = hackathons.find((hk) => hk._id === id);
      if (!h) return;
      reset({
        title: h.title,
        tagline: h.tagline,
        ...
        registrationStart: toLocalDateTime(h.registrationStart),
        registrationEnd: toLocalDateTime(h.registrationEnd),
        startDate: toLocalDateTime(h.startDate),
        endDate: toLocalDateTime(h.endDate),
        rules: h.rules?.map((r) => ({ value: r })) || [{ value: "" }],
        judgingCriteria: h.judgingCriteria || [{ name: "Innovation", weight: 25 }],
      });
    });
  }, [id, reset]);
  ...
  const onSubmit = async (values) => {
    try {
      const payload = {
        ...
        judgingCriteria: values.judgingCriteria
          .filter((c) => c.name)
          .map((c) => ({ ...c, weight: Number(c.weight) })),
      };
      await hackathonService.update(id, payload);
      toast.success("Hackathon updated!");
      navigate(`/dashboard/organizer/hackathons/${id}`);
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <p className="mt-1 text-sm text-[var(--ink-muted)]">Changes are saved immediately — no republishing needed.</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-6 flex flex-col gap-6">
        <Card className="p-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <Textarea label="Description" rows={5} error={errors.description?.message} {...register("description", { required: "Description is required" })} />
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Input label="Themes (comma-separated)" {...register("themes")} />
            </div>
          </div>
        </Card>
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Input label="Registration Start" type="datetime-local" {...register("registrationStart", { required: "Required" })} />
            <Input label="Registration End" type="datetime-local" {...register("registrationEnd", { required: "Required" })} />
            <Input label="Hackathon Start" type="datetime-local" {...register("startDate", { required: "Required" })} />
            <Input label="Hackathon End" type="datetime-local" {...register("endDate", { required: "Required" })} />
          </div>
        </Card>
          ...
          <div className="flex flex-col gap-2.5">
            {rulesArray.fields.map((field, i) => (
              <div key={field.id} className="flex gap-2">
                ...
                <button type="button" onClick={() => rulesArray.remove(i)} className="rounded-lg px-3 text-[var(--ink-muted)] hover:bg-danger/10 hover:text-danger">
                  <FiTrash2 size={15} />
                </button>
              </div>
            ))}
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Button type="button" variant="ghost" size="sm" icon={<FiPlus size={14} />} onClick={() => criteriaArray.append({ name: "", weight: 10 })}>
              Add criterion
            </Button>
          </div>
          <div className="flex flex-col gap-2.5">
            {criteriaArray.fields.map((field, i) => (
              <div key={field.id} className="flex gap-2">
                ...
                <button type="button" onClick={() => criteriaArray.remove(i)} className="rounded-lg px-3 text-[var(--ink-muted)] hover:bg-danger/10 hover:text-danger">
                  <FiTrash2 size={15} />
                </button>
              </div>
            ))}
